// ok i made it slightly better, pls star
...
parse_positive_int :: proc(s, name: string) -> int {
    ...
    if n < 0 {
        fail(fmt.tprintf("%s must be a positive integer", name), 2)
    }
    return n
}
...
parse_args :: proc(cfg: ^Config, args: []string) {
    ...
    if cfg.remote_name && cfg.output_path == "" && cfg.url != "" {
...
format_bytes :: proc(n: i64) -> string {
    ...
    switch {
    case n >= TB: return fmt.tprintf("%.2f TB", f64(n) / f64(TB))
    case n >= GB: return fmt.tprintf("%.2f GB", f64(n) / f64(GB))
    case n >= MB: return fmt.tprintf("%.2f MB", f64(n) / f64(MB))
    case n >= KB: return fmt.tprintf("%.2f KB", f64(n) / f64(KB))
    }
...
has_header :: proc(headers: [dynamic]string, name: string) -> bool {
    ...
    for h in headers {
        if len(h) >= len(prefix) && eq_ignore_case(h[:len(prefix)], prefix) {
            return true
        }
    }
    return false
...
// ─────────────────────────────────────────────────────────────
// JSON pretty printer, tiny and dependency-free
// ─────────────────────────────────────────────────────────────
...
build_header_list :: proc(cfg: ^Config) -> ^curl.slist {
    ...
    for h in cfg.headers {
        list = curl.slist_append(list, strings.clone_to_cstring(h))
    }

    // Use real USERAGENT option too, but keep the explicit header for easy override symmetry.
    if !has_header(cfg.headers, "User-Agent") {
        list = curl.slist_append(list, fmt.ctprintf("User-Agent: %s", cfg.user_agent))
    }
...
setup_handle :: proc(
    ...
    if has_body {
        ...
        if method == "GET" {
            // Body with GET is weird but not illegal. If user asked for it, obey.
            curl.easy_setopt(handle, .CUSTOMREQUEST, cstring("GET"))
        }
    }
    ...
    if resume_from > 0 {
        curl.easy_setopt(handle, .RESUME_FROM_LARGE, curl.off_t(resume_from))
    }
...
write_file_all :: proc(path: string, data: []u8) {
    ...
    if werr != nil || written != len(data) {
        fail(fmt.tprintf("failed writing '%s'", path))
    }
}
...
run_single :: proc(cfg: ^Config, method: string, payload: ^Payload) -> int {
    ...
    for attempt := 0; attempt < attempts; attempt += 1 {
        if attempt > 0 && !cfg.silent {
            fmt.eprintf("%s%s%s retry %d/%d after %dms\n", ANSI_YELLOW, SYM_WARN, ANSI_RESET, attempt, cfg.retry_count, cfg.retry_delay_ms)
    ...
    if final_result.curl_code != .E_OK {
        if cfg.show_error || !cfg.silent {
            fail(fmt.tprintf("request failed: %s", curl.easy_strerror(final_result.curl_code)), 1)
        }
        return 1
    }

    // curl --fail compatibility: do this before printing body.
    if cfg.fail_http && final_result.status >= 400 {
        if cfg.show_error || !cfg.silent {
            fmt.eprintf("%s%s:%s HTTP %d %s\n", ANSI_RED, APP_NAME, ANSI_RESET, final_result.status, status_label(final_result.status))
        }
        return 22
    }
    ...
    if pretty {
        print_response_line(method, cfg.url, final_result.status, final_result.duration, final_result.bytes)
    }

    if cfg.debug do print_timing(final_result.timing)

    if cfg.dump_headers_path != "" && len(final_head.buf) > 0 {
        write_file_all(cfg.dump_headers_path, final_head.buf[:])
    }
    ...
    if cfg.output_path != "" && !cfg.silent {
        fmt.eprintf("%s%s%s saved %s%s%s (%s)\n",
...
run_benchmark :: proc(cfg: ^Config, method: string, payload: ^Payload) -> int {
    ...
    for i := 0; i < cfg.bench_count; i += 1 {
        ...
        append(&times, ms)
        fmt.eprintf("  %s#%-4d%s %s%d%s %s\n",
            ANSI_DIM, i + 1, ANSI_RESET,
            status_color(result.status), result.status, ANSI_RESET,
            format_ms(ms))
    }
...
// ─────────────────────────────────────────────────────────────
// Bitch; i mean batch mode, real parallelism using libcurl multi
// ─────────────────────────────────────────────────────────────
...
run_batch :: proc(cfg: ^Config, method: string, payload: ^Payload) -> int {
    ...
    if mres != .OK && mres != .CALL_MULTI_PERFORM {
        fail(fmt.tprintf("curl multi perform failed: %s", curl.multi_strerror(mres)))
    }
    ...
    for completed < len(jobs) {
        ...
        for {
            ...
            if msg.msg == .DONE {
                ...
                if ok {
                    success += 1
                    fmt.eprintf("  %s%s%s %-4d %s%d%s %-9s %s%s%s %s\n",
                        ANSI_GREEN, SYM_OK, ANSI_RESET,
                        idx + 1,
                        status_color(job.status), job.status, ANSI_RESET,
                        format_duration(job.duration),
                        ANSI_DIM, format_bytes(job.body.total_written), ANSI_RESET,
                        job.url)
                } else {
                    failed += 1
                    if job.result != .E_OK {
                        fmt.eprintf("  %s%s%s %-4d curl %-8s %s%s%s\n",
                            ANSI_RED, SYM_FAIL, ANSI_RESET,
                            idx + 1,
                            format_duration(job.duration),
                            ANSI_DIM, job.url, ANSI_RESET)
...
append_str :: proc(buf: ^[dynamic]u8, s: string) {
    if len(s) > 0 {
        append(buf, ..raw_data(s)[:len(s)])
    }
}

send_str :: proc(sock: net.TCP_Socket, s: string) -> bool {
    if len(s) == 0 do return true
    _, err := net.send_tcp(sock, raw_data(s)[:len(s)])
    return err == nil
}

send_bytes :: proc(sock: net.TCP_Socket, data: []u8) -> bool {
...
server_send_response :: proc(sock: net.TCP_Socket, code: int, content_type: string, body: []u8, extra: string = "") {
    header := server_header(code, content_type, len(body), extra)
    send_str(sock, header)
    send_bytes(sock, body)
}

server_send_text :: proc(sock: net.TCP_Socket, code: int, content_type: string, body: string, extra: string = "") {
    header := server_header(code, content_type, len(body), extra)
    send_str(sock, header)
    send_str(sock, body)
}
...
server_mime :: proc(path: string) -> string {
    ...
    case strings.has_suffix(lower, ".tar"):
        return "application/x-tar"
    case strings.has_suffix(lower, ".gz"):
        return "application/gzip"
    }
    return "application/octet-stream"
}

html_escape_to :: proc(buf: ^[dynamic]u8, s: string) {
    for ch in s {
        switch ch {
        case '&': append_str(buf, "&amp;")
        case '<': append_str(buf, "&lt;")
    ...
    for line in lines {
        ...
        if idx := strings.index(trimmed, ":"); idx >= 0 {
            ...
            if eq_ignore_case(name, "Content-Length") {
                return int(strconv.parse_int(value) or_else 0)
            }
        }
    }
    return 0
...
parse_http_request :: proc(raw: []u8, header_end: int) -> (Http_Request, bool) {
    ...
    if body_start < len(raw) {
        body = raw[body_start:]
        if content_length > 0 && len(body) > content_length {
...
server_read_request :: proc(sock: net.TCP_Socket, max_request: int) -> (Http_Request, [dynamic]u8, bool) {
    ...
    for {
        ...
        if header_end >= 0 && len(raw) >= header_end + 4 + content_length {
            break
        }
    }
...
server_send_file :: proc(sock: net.TCP_Socket, path: string, method: string) {
    ...
    f, err := os.open(path)
    if err != nil {
        // Headers may already be gone, so just close. This should be rare after stat succeeded.
        return
    }
    defer os.close(f)
    ...
    for {
        ...
        if n > 0 {
            if !send_bytes(sock, buf[:n]) do break
        }
...
server_send_directory_listing :: proc(sock: net.TCP_Socket, req_path, fs_path: string) {
    ...
    append_str(&body, "<!doctype html><html><head><meta charset=\"utf-8\"><meta name=\"viewport\" content=\"width=device-width,initial-scale=1\">")
    append_str(&body, "<title>volt directory</title><style>body{font-family:system-ui,sans-serif;margin:2rem;max-width:900px}a{text-decoration:none}table{width:100%;border-collapse:collapse}td{padding:.35rem;border-bottom:1px solid #ddd}.dim{color:#777}</style></head><body>")
    append_str(&body, "<h1>volt directory: ")
    html_escape_to(&body, req_path)
    append_str(&body, "</h1><p class=\"dim\">Endpoints: <code>/__volt/health</code>, <code>/__volt/manifest</code>, <code>/__volt/echo</code></p><table>")

    base := req_path
    if !strings.has_suffix(base, "/") do base = fmt.tprintf("%s/", base)

    append_str(&body, "<tr><td><a href=\"../\">../</a></td><td></td></tr>")
    for info in infos {
...
server_handle_upload :: proc(sock: net.TCP_Socket, cfg: ^Config, req: Http_Request) {
    ...
    decoded, ok := net.percent_decode(name, context.temp_allocator)
...
server_handle_client :: proc(sock: net.TCP_Socket, peer: net.Endpoint, cfg: ^Config, start_time: time.Time) {
...
app_main :: proc() -> int {
    ...
    method := cfg.method
    if method == "" {
        method = payload_len(&payload) > 0 ? "POST" : "GET"
    }
    ...
    if cfg.batch_file != "" {
        return run_batch(&cfg, method, &payload)
    }
    if cfg.bench_count > 0 {
        return run_benchmark(&cfg, method, &payload)
    }
